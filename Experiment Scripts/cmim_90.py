...
# TT --> Training time
print ("FS runtime, Base NB TT, 0.9 NB TT, Base NB accuracy, 0.9 NB accuracy,",
    "Base KNN TT, 0.9 KNN TT, Base KNN accuracy, 0.9 KNN accuracy,",
    "Base LDA TT, 0.9 LDA TT, Base LDA accuracy, 0.9 LDA accuracy,",
    "Base MLP TT, 0.9 MLP TT, Base MLP accuracy, 0.9 MLP accuracy")
...
for data in openml_datasets:
  ...
  for train_index, test_index in kfold.split(X):
    ...
    start = time()
    idx,_,_ = CMIM.cmim(X[train_index], y[train_index], n_selected_features=num_fea9)
